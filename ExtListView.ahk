...
; .................: December 2020 -      - Added methods InitializeFromHwnd, EnsureVisible, and GetItemRect, and added
; .................:                        "classnn" optional parameter to Initialize.
; NOTE.............: While 64-bit compatibility is claimed, we still experienced some problems with the library that 
; .................: were solved by switching to a 32-bit version of AHK.
; ----------------------------------------------------------------------------------------------------------------------
...
; Description ..: Initialize the object containing ListView's related data based on the name of the parent window.
; Parameters ...: sWnd      - Title of the window containing the ListView.
; ..............: classnn   - Classnn of the ListView to be retrieved, if the window contains more than one.
...
; ----------------------------------------------------------------------------------------------------------------------
ExtListView_Initialize(sWnd, classnn="SysListView321", szReadBuf:=1024, sEnc:="CP0") {
    objLV            := Object()
    objLV.swnd       := sWnd
    ...
    ControlGet, hLv, Hwnd,, %classnn%, % "ahk_id " objLV.hwnd
    objLV.hlv := hLv
...
; ----------------------------------------------------------------------------------------------------------------------
; Function .....: ExtListView_InitializeFromHwnd
; Description ..: Initialize the object containing ListView's related data based on the window handle of the ListView.
; Parameters ...: hwnd      - Window handle of the ListView itself (not of any parent windows).
; ..............: szReadBuf - Size of the buffer used for reading the target process memory. It must be capable enough
; ..............:             to hold the longest cell in the ListView.
; ..............: sEnc      - Target ListView's encoding. "CP0" for ANSI, "UTF-8" or "UTF-16" for Unicode.
; Return .......: objLV            - External ListView initialized object with the following keys:
...
; ..............: objLV.szreadbuf  - Size of the read buffer.
; ----------------------------------------------------------------------------------------------------------------------
ExtListView_InitializeFromHwnd(hwnd, szReadBuf:=1024, sEnc:="CP0"){
    objLV            := Object()
    objLV.hlv        := hwnd
    objLV.szwritebuf := (A_OSVersion == "WIN_XP") ? 52 + (A_PtrSize-4)*5 : 60 + (A_PtrSize-4)*7 ; Size of LVITEM
    objLV.szreadbuf  := szReadBuf
    ...
    DllCall( "GetWindowThreadProcessId", Ptr,objLV.hlv, PtrP,dwProcessId )
    ; PROCESS_VM_OPERATION = 0x0008, PROCESS_VM_READ = 0x0010, PROCESS_VM_WRITE = 0x0020.
    If ( !(objLV.hproc := DllCall( "OpenProcess", UInt,0x0008|0x0010|0x0020, Int,0, UInt,dwProcessId )) )
        Throw Exception("objLV.hproc: error opening process", "OpenProcess", "LastError: " A_LastError)
...
; ----------------------------------------------------------------------------------------------------------------------
; Function .....: ExtListView_EnsureVisible
; Description ..: Ensures that a particular row of the list view is visible, scrolling the view if necessary.
; Parameters ...: objLV - External ListView initialized object.
; ..............: nRow - index of row to make visible (0-indexed)
; ----------------------------------------------------------------------------------------------------------------------
ExtListView_EnsureVisible(ByRef objLV, nRow)
{
	SendMessage, 0x1000 + 19, nRow, 1,, % "ahk_id " objLV.hlv
}

; ----------------------------------------------------------------------------------------------------------------------
; Function .....: ExtListView_GetItemRect
; Description ..: Returns the coordinates of a particular row of the ListView, relative to the view's top-left corner
; Parameters ...: objLV - External ListView initialized object.
; ..............: nRow - index of row to make visible (0-indexed)
; Return .......: [x1, y1, x2, y2] - coordinates of the row's bounding box, with the top-left corner at (x1, y1)
; ..............:                    and the bottom-right corner at (x2, y2)
; ----------------------------------------------------------------------------------------------------------------------
ExtListView_GetItemRect(ByRef objLV, nRow)
{
	VarSetCapacity(rect, 16, 0)
	If ( !DllCall( "WriteProcessMemory", Ptr,objLV.hproc, Ptr,objLV.pwritebuf, Ptr, &rect, UInt, 16
		, UInt,0 ) )
		Throw Exception("objLV.pwritebuf: error writing memory", "WriteProcessMemory", "LastError: " A_LastError)
	SendMessage, 0x1000 + 14, nRow, objLV.pwritebuf,, % "ahk_id " objLV.hlv
	If ( !DllCall( "ReadProcessMemory", "Ptr", objLV.hproc, "Ptr", objLV.pwritebuf, "Ptr", &rect, "UInt", 16, "Ptr", 0 ) )
		Throw Exception("objLV.preadbuf: error reading memory", "ReadProcessMemory", "LastError: " A_LastError)

	return [NumGet(rect, 0, "Int"), NumGet(rect, 4, "Int"), NumGet(rect, 8, "Int"), NumGet(rect, 12, "Int")]
}
